...
typedef NameMap::iterator NameIterator;
NameMap names_ = {
  ...
  { "lightcyan", 0xe0ffff }, { "lightgoldenrodyellow", 0xfafad2 }, { "lightgrey", 0xd3d3d3 }, { "lightgreen", 0x90ee90 },
  ...
  { "palevioletred", 0xd87093 }, { "papayawhip", 0xffefd5 }, { "peachpuff", 0xffdab9 }, { "peru", 0xcd853f },
...
bool text(uint32_t& color, std::string text) {
  ...
  if (l > 0 && text.at(0) == '#') {
    ...
    NameIterator it = ::gos::color::names_.find(text);
    if (it != ::gos::color::names_.end()) {
      color = it->second;
      return true;
    } else if (::gos::text::are::all::hex(text)) {
      long n = strtol(text.c_str(), NULL, 16);
      color = static_cast<uint32_t>(n);
      return true;
    }
